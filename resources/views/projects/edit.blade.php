...
@section('title', 'Editar Orçamento - Gestor de Freelas')
...
<div x-data="projectForm()" class="space-y-6">
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-8 items-start">
        ...
        <!-- Formulário de Edição (Esquerda - 2 Colunas) -->
        <div class="bg-white rounded-[5px] border border-slate-200 p-5 sm:p-8 space-y-6 xl:col-span-2 shadow-sm">
            <div>
                <h2 class="text-xl font-bold text-slate-900">Editar Orçamento</h2>
                <p class="text-xs text-slate-400 mt-1">Altere as informações abaixo para atualizar a proposta ou contrato do projeto.</p>
            </div>

            <form action="{{ route('projects.update', $project->id) }}" method="POST" class="space-y-6" @submit="submitForm($event)">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="border border-slate-200 rounded-[5px] overflow-hidden focus-within:ring-2 focus-within:ring-primary-500/20 focus-within:border-primary-500 transition-all bg-white">
                        ...
                        <!-- Área Editável com Auto-resize e preenchimento inicial -->
                        <div x-ref="editor" 
                             contenteditable="true" 
                             x-init="$refs.editor.innerHTML = description || ''; $nextTick(() => { $refs.editor.style.height = 'auto'; $refs.editor.style.height = $refs.editor.scrollHeight + 'px' })"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <!-- Data do Orçamento -->
                    <div class="space-y-1.5 relative" x-data="customDatePicker('budget_date', budgetDate, true)">
                    ...
                    <!-- Data de Validade -->
                    <div class="space-y-1.5 relative" x-data="customDatePicker('expiration_date', expDate, false)" @budget-date-changed.window="updateExpiration($event.detail)">
                ...
                <div class="space-y-2">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        ...
                        <label class="flex items-center gap-2.5 p-3 rounded-[5px] border cursor-pointer transition-all select-none"
                            ...
                            <span class="w-3.5 h-3.5 rounded-full bg-purple-555 inline-block bg-purple-500"></span>
                ...
                <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                    <a href="{{ route('projects.index') }}" class="px-6 py-3 border border-slate-200 text-slate-600 hover:text-slate-800 hover:bg-slate-50 transition-colors font-semibold rounded-[5px] text-sm">
                        Cancelar
                    </a>
...
<script>
    ...
    function projectForm() {
        return {
            ...
            selectedClient: @json($project->client),
            newClientName: '',
            clientQuery: '',
            clientDropdown: false,

            authorQuery: '',
            selectedAuthors: @json($project->authors->map(fn($a) => ['id' => $a->id, 'name' => $a->name, 'is_new' => false])),
            authorDropdown: false,

            totalValue: 'R$ ' + (@json($project->total_value) * 1).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 }),
            term: @json($project->term),
            sliderValue: @json($project->initial_payment_percent),
            status: @json($project->status),
            additionalInfo: @json($project->additional_info ?? ''),

            budgetDate: @json($project->budget_date),
            expDate: @json($project->expiration_date),

            init() {
                // Escuta os eventos globais disparados pelo datepicker
