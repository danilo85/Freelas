    ...
    <form action="{{ route('portfolio.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6" @submit.prevent="submitForm($event)">
        ...
        <div x-show="activeTab === 'galeria'" class="bg-white border border-slate-200 p-6 rounded-[5px] shadow-sm space-y-6" x-cloak>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
                ...
                <!-- Coluna Esquerda: Thumbnail Dropzone (Tamanho Igual e Compacto) -->
                <div class="space-y-2">
                    <label class="text-xs font-bold text-slate-800 uppercase tracking-wider block">
                        Imagem de Capa (Thumbnail) <span class="text-red-500">*</span>
                    </label>
                        ...
                        <label :class="dragOver ? 'border-primary-500 bg-primary-50/20' : 'border-slate-200 bg-slate-50'"
                            ...
                            <template x-if="!thumbPreview">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div class="text-left">
                                        ...
                                        <p class="text-[10px] text-slate-400">WebP, PNG ou JPG (Max 20MB)</p>
                                    </div>
                                </div>
                            </template>
                            ...
                            <template x-if="thumbPreview">
                                <div class="relative w-full h-24 rounded overflow-hidden">
                                    ...
                                    <span class="absolute top-1 right-1 bg-slate-900/80 text-white text-[9px] font-bold px-1.5 py-0.5 rounded">Capa Selecionada</span>
                                </div>
                            </template>
                ...
                <!-- Coluna Direita: Galeria Dropzone (Tamanho Igual ao Capa) -->
                <div class="space-y-2">
                    <label class="text-xs font-bold text-slate-800 uppercase tracking-wider block">
                        ...
                        <label :class="dragOver ? 'border-primary-500 bg-primary-50/20' : 'border-slate-200 bg-slate-50'"
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="text-left">
                                    ...
                                    <p class="text-[10px] text-slate-400">Envie múltiplos arquivos para a galeria</p>
                                </div>
                            </div>
            ...
            <!-- Lista e Reordenação Visual por Arraste e Solte (Drag & Drop) -->
            <div class="space-y-3 pt-2 border-t border-slate-100">
                <div class="flex items-center justify-between">
                    <h5 class="text-xs font-bold text-slate-800 uppercase tracking-wider">
                        Sequência de Fotos da Galeria (Arraste para Reordenar)
                    </h5>
                    <span class="text-[10px] text-slate-400 font-medium">
                        💡 Clique e segure uma foto para mover de posição
                    </span>
                </div>

                <!-- Grid com HTML5 Drag & Drop -->
                <template x-if="galleryFiles.length > 0">
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 max-h-[400px] overflow-y-auto p-1">
                            ...
                            <div draggable="true"
                                ...
                                <div class="relative bg-slate-900/80 text-white px-2 py-1.5 flex items-center justify-between z-10">
                                    ...
                                    <div class="flex items-center gap-1">
                                        <button type="button" @click="moveUp(index)" :disabled="index === 0" class="w-5 h-5 rounded bg-slate-800 text-slate-200 flex items-center justify-center hover:bg-slate-700 disabled:opacity-30 border-0" title="Mover para esquerda">
                                            ‹
                                        </button>
                                        <button type="button" @click="moveDown(index)" :disabled="index === galleryFiles.length - 1" class="w-5 h-5 rounded bg-slate-800 text-slate-200 flex items-center justify-center hover:bg-slate-700 disabled:opacity-30 border-0" title="Mover para direita">
                                            ›
                                        </button>
                                    </div>
                ...
                <div class="p-8 text-center text-slate-400 text-xs italic border border-slate-150 rounded-[5px] bg-slate-50/30" x-show="galleryFiles.length === 0">
                    Nenhuma foto adicional adicionada à galeria ainda.
                </div>
            </div>
