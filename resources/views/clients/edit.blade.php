...
@section('title', 'Editar Cliente - Gestor de Freelas')
@section('page_title', 'Editar Cliente')

@section('content')
<div x-data="clientEditForm()" class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div class="bg-white rounded-[5px] border border-slate-200 p-5 sm:p-8 space-y-6 lg:col-span-2">
            <div>
                <h2 class="text-xl font-bold text-slate-900">Editar Cliente</h2>
                <p class="text-xs text-slate-400 mt-1">Altere as informações abaixo para atualizar os dados cadastrais do cliente.</p>
            </div>
            ...
            <form action="{{ route('clients.update', $client->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
                    <a href="{{ route('clients.index') }}" class="px-6 py-3 border border-slate-200 text-slate-600 hover:text-slate-800 hover:bg-slate-50 transition-colors font-semibold rounded-[5px] text-sm">
                        Cancelar
                    </a>
...
<script>
    function clientEditForm() {
        return {
            ...
            document: '{{ addslashes($client->document) }}',
            avatarUrl: '{{ $client->avatar ? asset("storage/" . $client->avatar) : "" }}',
            avatarName: '{{ $client->avatar ? basename($client->avatar) : "" }}',
            dragOver: false,
