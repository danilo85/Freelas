        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="space-y-1">
                ...
                <select 
                    ...
                    id="category_id" 
                    required
                    data-slimselect-initialized="true"
        ...
        <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                <label class="border rounded-[5px] p-3 flex items-center justify-center gap-2 cursor-pointer text-xs font-bold transition-all"
                       :class="repeatType === 'single' ? 'border-primary-500 bg-primary-50/20 text-primary-700 dark:text-primary-400 dark:bg-primary-950/20 ring-2 ring-primary-500/10 dark:ring-primary-400/20' : 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-650 dark:text-slate-350'">
                    <input type="radio" name="repeat_type" value="single" x-model="repeatType" class="hidden" />
                    📅 Lançamento Único
                </label>
                <label class="border rounded-[5px] p-3 flex items-center justify-center gap-2 cursor-pointer text-xs font-bold transition-all"
                       :class="repeatType === 'installments' ? 'border-primary-500 bg-primary-50/20 text-primary-700 dark:text-primary-400 dark:bg-primary-950/20 ring-2 ring-primary-500/10 dark:ring-primary-400/20' : 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-650 dark:text-slate-350'">
                    <input type="radio" name="repeat_type" value="installments" x-model="repeatType" class="hidden" />
                    💳 Compras Parceladas
                </label>
                <label class="border rounded-[5px] p-3 flex items-center justify-center gap-2 cursor-pointer text-xs font-bold transition-all"
                       :class="repeatType === 'recurring' ? 'border-primary-500 bg-primary-50/20 text-primary-700 dark:text-primary-400 dark:bg-primary-950/20 ring-2 ring-primary-500/10 dark:ring-primary-400/20' : 'border-slate-200 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800/50 text-slate-650 dark:text-slate-350'">
    ...
    function transactionForm() {
        return {
            ...
            get filteredCategories() {
                const isEntrada = this.type === 'entrada';
                return this.categories.filter(c => {
                    if (c.type === 'ambos') return true;
                    if (isEntrada) {
                        return c.type === 'receita' || c.type === 'entrada';
                    } else {
                        return c.type === 'despesa' || c.type === 'saida';
                    }
                });
            },
